          ...
          <div class="p-6">
            <div class="flex items-center justify-between mb-6">
              <div>
                ...
                <p class="text-gray-600">Manage university courses and program offerings</p>
              </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
              <div class="bg-blue-50 p-4 rounded-lg border border-blue-200">
                <div class="flex items-center">
                  <div class="p-2 bg-blue-100 rounded-lg">
                    <BookIcon class="w-6 h-6 text-blue-600" />
                  </div>
                  <div class="ml-3">
              ...
              <div class="bg-green-50 p-4 rounded-lg border border-green-200">
                <div class="flex items-center">
                  ...
                  <div class="ml-3">
                    <p class="text-sm font-medium text-green-600">Bachelor's</p>
                    <p class="text-2xl font-bold text-green-900">{{ stats.bachelors }}</p>
                  </div>
                </div>
              </div>
              ...
              <div class="bg-purple-50 p-4 rounded-lg border border-purple-200">
                <div class="flex items-center">
                  ...
                  <div class="ml-3">
                    <p class="text-sm font-medium text-purple-600">Master's</p>
                    <p class="text-2xl font-bold text-purple-900">{{ stats.masters }}</p>
                  </div>
                </div>
              </div>
              ...
              <div class="bg-yellow-50 p-4 rounded-lg border border-yellow-200">
                <div class="flex items-center">
                  <div class="p-2 bg-yellow-100 rounded-lg">
                    <CertificateIcon class="w-6 h-6 text-yellow-600" />
                  </div>
                  <div class="ml-3">
                    <p class="text-sm font-medium text-yellow-600">PhD</p>
                    <p class="text-2xl font-bold text-yellow-900">{{ stats.phd }}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="mb-4 flex flex-col sm:flex-row gap-4">
              ...
              <div class="flex gap-2">
                ...
                >
                  <option v-for="option in degreeLevelOptions" :key="option.value" :value="option.value">
                    {{ option.label }}
                  </option>
                </select>
                <select
                  ...
                  class="w-40 px-3 py-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500"
                >
                  <option v-for="option in facultyOptions" :key="option.value" :value="option.value">
                    {{ option.label }}
                  </option>
                </select>
                <select
            ...
            <div class="bg-white rounded-lg border">
              <div class="overflow-x-auto">
                <table class="w-full">
                  ...
                  <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-for="course in filteredCourses" :key="course.name" class="hover:bg-gray-50">
                      <td class="px-6 py-4">
                        <div>
                      ...
                      <td class="px-6 py-4">
                        <div class="text-sm text-gray-900">{{ course.university }}</div>
                      </td>
                      <td class="px-6 py-4">
                        <div>
                          <div class="text-sm font-medium text-gray-900">{{ course.faculty || 'N/A' }}</div>
                          <div class="text-sm text-gray-500">{{ course.degree_level || 'N/A' }}</div>
                        </div>
                      </td>
                      <td class="px-6 py-4">
                        <div class="text-sm text-gray-900">{{ course.country || 'N/A' }}</div>
                      </td>
                      <td class="px-6 py-4">
                        <div class="flex space-x-2">
                          <Button size="sm" variant="outline" @click="viewCourse(course)">
                            View
                          </Button>
                          <Dropdown :options="getActionOptions(course)">
              ...
              <div v-if="filteredCourses.length === 0" class="text-center py-8">
                ...
                <h3 class="text-sm font-medium text-gray-900 mb-1">No courses found</h3>
                <p class="text-sm text-gray-500">Get started by adding your first course.</p>
              </div>
            </div>
          </div>
        </main>
...
const GraduationCapIcon = {
  template: `<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l9-5-9-5-9 5 9 5z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14V2.5"></path></svg>`
}
...
export default {
  ...
  data() {
    return {
      ...
      navigation: [
        ...
        { name: 'Course Management', href: '/course-management', icon: BookIcon, current: true },
    ...
    filteredCourses() {
      ...
      return this.courses.filter(course => {
        const matchesSearch = !this.searchTerm ||
          ...
          course.university?.toLowerCase().includes(this.searchTerm.toLowerCase()) ||
          course.faculty?.toLowerCase().includes(this.searchTerm.toLowerCase())

        const matchesDegreeLevel = !this.selectedDegreeLevel || course.degree_level === this.selectedDegreeLevel
        const matchesFaculty = !this.selectedFaculty || course.faculty === this.selectedFaculty
        const matchesCountry = !this.selectedCountry || course.country === this.selectedCountry

        return matchesSearch && matchesDegreeLevel && matchesFaculty && matchesCountry
      })
    },

    stats() {
      if (!this.courses) {
        return { total: 0, bachelors: 0, masters: 0, phd: 0 }
      }

      const data = this.courses
      return {
        total: data.length,
        bachelors: data.filter(course => course.degree_level === "Bachelor's").length,
        masters: data.filter(course => course.degree_level === "Master's").length,
        phd: data.filter(course => course.degree_level === 'PhD').length
      }
    }
  },
  mounted() {
  ...
  methods: {
    ...
    loadCourses() {
      const resource = createResource({
        ...
        params: {
          doctype: 'Course',
          fields: [
            'name', 'course_name', 'university', 'faculty', 'country', 'degree_level',
            'creation', 'modified'
          ],
    ...
    getActionOptions(course) {
      return [
        ...
        },
        {
          ...
          icon: 'file-text',
          onClick: () => this.viewRequirements(course)
        },
        {
          label: 'Duplicate',
          icon: 'copy',
          onClick: () => this.duplicateCourse(course)
        },
        {
          label: 'Delete',
          icon: 'trash-2',
    ...
    createNew() {
      window.location.href = `/app/course/new`
    },

    viewCourse(course) {
      window.location.href = `/app/course/${course.name}`
    },
    ...
    viewRequirements(course) {
      console.log('View requirements for:', course.name)
    },

    duplicateCourse(course) {
      if (confirm(`Create a copy of ${course.course_name}?`)) {
