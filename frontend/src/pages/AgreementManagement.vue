    ...
    <div class="flex-1 lg:pl-0">
      ...
      <header class="bg-white shadow-sm border-b border-gray-200">
        <div class="flex items-center justify-between h-16 px-6">
          <div class="flex items-center">
            <h2 class="text-2xl font-bold text-gray-900">Agreement Management</h2>
          </div>
      ...
      <main class="p-6">
        ...
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 mb-8">
            <div class="overflow-hidden rounded-lg bg-white px-4 py-5 shadow sm:p-6">
              <dt class="truncate text-sm font-medium text-gray-500">Total Agreements</dt>
            ...
            <div class="overflow-hidden rounded-lg bg-white px-4 py-5 shadow sm:p-6">
              ...
              <dd class="mt-1 text-3xl font-semibold tracking-tight text-blue-600">{{ stats.draft }}</dd>
            </div>
            <div class="overflow-hidden rounded-lg bg-white px-4 py-5 shadow sm:p-6">
              <dt class="truncate text-sm font-medium text-gray-500">Under Review</dt>
              <dd class="mt-1 text-3xl font-semibold tracking-tight text-yellow-600">{{ stats.underReview }}</dd>
            </div>
            <div class="overflow-hidden rounded-lg bg-white px-4 py-5 shadow sm:p-6">
        ...
        <div class="bg-white shadow rounded-lg overflow-hidden">
            <div class="px-4 py-5 sm:p-6">
              <div v-if="loading" class="text-center py-8">
                ...
                <p class="mt-2 text-sm text-gray-500">Loading agreements...</p>
              </div>

              <div v-else-if="agreements.length === 0" class="text-center py-12">
                <FeatherIcon name="file-text" class="mx-auto h-12 w-12 text-gray-400" />
                <h3 class="mt-2 text-sm font-semibold text-gray-900">No agreements found</h3>
                <p class="mt-1 text-sm text-gray-500">Get started by creating a new agreement.</p>
                <div class="mt-6">
                  <Button variant="solid" @click="createNew">
                    <template #prefix>
                      <FeatherIcon name="plus" class="h-4 w-4" />
              ...
              <div v-else class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-for="agreement in agreements" :key="agreement.name" class="hover:bg-gray-50">
                      ...
                      <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm text-gray-900">{{ agreement.provider_name }}</div>
                        <div class="text-sm text-gray-500">{{ agreement.client_name }}</div>
                      </td>
                      <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm text-gray-900">{{ agreement.service_type }}</div>
                      </td>
                      <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm text-gray-900">
                          {{ formatCurrency(agreement.total_amount, agreement.currency) }}
                        </div>
                      </td>
                      <td class="px-6 py-4 whitespace-nowrap">
                        <Badge
                          :variant="getStatusVariant(agreement.agreement_status)"
                          :label="agreement.agreement_status"
                        />
                      </td>
                      ...
                      <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <Dropdown :options="getActionOptions(agreement)" />
                      </td>
                    </tr>
                  </tbody>
                </table>
...
export default {
  ...
  setup() {
    ...
    const navigation = [
      ...
      { name: 'Agreement Management', href: '/agreement-management', icon: ClipboardListIcon, current: true },
    ...
    const stats = computed(() => {
      const total = agreements.value.length
      const draft = agreements.value.filter(a => a.agreement_status === 'Draft').length
      const underReview = agreements.value.filter(a => a.agreement_status === 'Under Review').length
      const accepted = agreements.value.filter(a => a.agreement_status === 'Accepted').length

      return { total, draft, underReview, accepted }
    })
    ...
    const loadAgreements = () => {
      ...
      const resource = createResource({
        ...
        params: {
          doctype: 'Agreement',
          fields: [
            'name', 'agreement_title', 'offer', 'agreement_template',
            'agreement_date', 'agreement_status', 'provider', 'provider_name',
            'client', 'client_name', 'service_type', 'total_amount', 'currency',
            'valid_until', 'signed_date', 'creation', 'modified'
          ],
          filters: buildFilters(),
          order_by: 'agreement_date desc'
        }
      })
    ...
    const buildFilters = () => {
      ...
      if (searchQuery.value) {
        filters.push(['agreement_title', 'like', `%${searchQuery.value}%`])
      }

      if (selectedStatus.value) {
        filters.push(['agreement_status', '=', selectedStatus.value])
      }

      if (providerFilter.value) {
        filters.push(['provider_name', 'like', `%${providerFilter.value}%`])
      }
    ...
    const getActionOptions = (agreement) => {
      return [
        {
          label: 'View',
        ...
        {
          label: 'Duplicate',
          onClick: () => duplicateAgreement(agreement)
        },
        {
          label: 'Delete',
    ...
    const viewAgreement = (agreement) => {
      console.log('View agreement:', agreement.name)
    }

    const editAgreement = (agreement) => {
      console.log('Edit agreement:', agreement.name)
    }

    const duplicateAgreement = (agreement) => {
      console.log('Duplicate agreement:', agreement.name)
    }

    const deleteAgreement = (agreement) => {
      console.log('Delete agreement:', agreement.name)
    }
