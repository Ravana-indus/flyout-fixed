<template>
  <div class="min-h-screen bg-gray-50 flex">
      ...
      <main class="p-6">
          ...
          <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
              ...
              <div class="ml-4">
                ...
                <p class="text-2xl font-bold text-gray-900">{{ stats.completedJobs }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
              ...
              <div class="ml-4">
                ...
                <p class="text-2xl font-bold text-gray-900">{{ stats.pendingJobs }}</p>
              </div>
            </div>
          </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="lg:col-span-2 bg-white rounded-lg shadow">
            ...
            <div class="p-6">
              <div class="space-y-4">
                <div v-for="job in recentJobs" :key="job.id" class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                  <div class="flex items-center">
                    <div class="flex-shrink-0">
                      <div :class="[
                        'w-10 h-10 rounded-full flex items-center justify-center',
                        job.status === 'completed' ? 'bg-green-100' : 'bg-yellow-100'
                      ]">
                        <svg :class="[
                          'w-5 h-5',
                    ...
                    <div class="ml-4">
                      ...
                      <p class="text-xs text-gray-500">{{ job.customer }} • {{ job.date }}</p>
                    </div>
                  </div>
                  <div class="text-right">
                    <p class="text-sm font-medium text-gray-900">${{ job.amount }}</p>
                    <p :class="[
                      'text-xs',
                      job.status === 'completed' ? 'text-green-600' : 'text-yellow-600'
                    ]">{{ job.status }}</p>
          ...
          <div class="bg-white rounded-lg shadow">
            ...
            <div class="p-6">
              <div class="space-y-3">
                <Button @click="showNewJobDialog = true" class="w-full justify-center">
                  <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <Button variant="outline" @click="showProfileDialog = true" class="w-full justify-center">
                  <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                  </svg>
                  Edit Profile
                ...
                <Button variant="outline" @click="showSettingsDialog = true" class="w-full justify-center">
                  <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <Button variant="outline" @click="showReportsDialog = true" class="w-full justify-center">
                  <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- New Job Dialog -->
    <Dialog title="Create New Job" v-model="showNewJobDialog" size="lg">
      <div class="p-6">
        <form @submit.prevent="createNewJob" class="space-y-4">
          <div>
          ...
          <div class="flex justify-end space-x-3 pt-4">
            <Button variant="outline" @click="showNewJobDialog = false">Cancel</Button>
            <Button type="submit" :loading="creatingJob">Create Job</Button>
          </div>
        </form>
      </div>
    </Dialog>
...
export default {
  ...
  methods: {
    ...
    async createNewJob() {
      ...
      try {
        await this.$resources.createJob.submit(this.newJob)
      } catch (error) {
