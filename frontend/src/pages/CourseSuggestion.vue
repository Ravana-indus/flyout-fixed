        ...
        <main class="flex-1 relative z-0 overflow-y-auto focus:outline-none">
          <div class="p-6">
            <div class="flex items-center justify-between mb-6">
              <div>
                <h1 class="text-2xl font-semibold text-gray-900">Course Suggestions</h1>
                <p class="text-gray-600">Manage course suggestions and approvals</p>
              </div>
            ...
            <div class="mb-4 flex flex-col sm:flex-row gap-4">
              ...
              <div class="flex gap-2">
                ...
                <select
                  ...
                  class="w-40 px-3 py-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500"
                >
                  <option v-for="option in universityOptions" :key="option.value" :value="option.value">
                    {{ option.label }}
                  </option>
                </select>
              </div>
            ...
            <div class="bg-white rounded-lg border">
              <div class="overflow-x-auto">
                <table class="w-full">
                  ...
                  <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-for="suggestion in filteredSuggestions" :key="suggestion.name" class="hover:bg-gray-50">
                      <td class="px-6 py-4">
                        <div>
                          <div class="text-sm font-medium text-gray-900">{{ suggestion.suggested_course_name }}</div>
                          <div class="text-sm text-gray-500">{{ suggestion.name }}</div>
                        </div>
                      </td>
                      <td class="px-6 py-4">
                        <div class="text-sm text-gray-900">{{ suggestion.university }}</div>
                      </td>
                      <td class="px-6 py-4">
                        <div class="text-sm text-gray-900">{{ suggestion.faculty || 'N/A' }}</div>
                      </td>
                      <td class="px-6 py-4">
                        <span
                          :class="[
                      ...
                      <td class="px-6 py-4">
                        <div class="flex space-x-2">
                          <Button size="sm" variant="outline" @click="viewSuggestion(suggestion)">
                            View
                          </Button>
                          <Button
                          ...
                          <Dropdown :options="getActionOptions(suggestion)">
              ...
              <div v-if="filteredSuggestions.length === 0" class="text-center py-8">
                ...
                <h3 class="text-sm font-medium text-gray-900 mb-1">No suggestions found</h3>
                <p class="text-sm text-gray-500">Get started by adding your first course suggestion.</p>
              </div>
            </div>
          </div>
        </main>
...
export default {
  ...
  data() {
    return {
      ...
      navigation: [
        ...
        { name: 'Course Suggestions', href: '/course-suggestions', icon: LightBulbIcon, current: true },
  ...
  computed: {
    filteredSuggestions() {
      ...
      return this.suggestions.filter(suggestion => {
        const matchesSearch = !this.searchTerm ||
          suggestion.suggested_course_name?.toLowerCase().includes(this.searchTerm.toLowerCase()) ||
          suggestion.university?.toLowerCase().includes(this.searchTerm.toLowerCase()) ||
          suggestion.faculty?.toLowerCase().includes(this.searchTerm.toLowerCase())

        const matchesStatus = !this.selectedStatus || suggestion.status === this.selectedStatus
        const matchesUniversity = !this.selectedUniversity || suggestion.university === this.selectedUniversity
    ...
    stats() {
      ...
      return {
        total: data.length,
        pending: data.filter(suggestion => suggestion.status === 'Pending' || !suggestion.status).length,
        approved: data.filter(suggestion => suggestion.status === 'Approved').length,
        rejected: data.filter(suggestion => suggestion.status === 'Rejected').length
      }
    }
  },
  mounted() {
    console.log('Loading Course Suggestions data...')
  ...
  methods: {
    ...
    loadSuggestions() {
      const resource = createResource({
        ...
        params: {
          doctype: 'Course Suggestion',
          fields: [
            'name', 'university', 'suggested_course_name', 'faculty', 'provider', 'status', 'master_course',
            'creation', 'modified'
          ],
    ...
    loadUniversityOptions() {
      ...
      resource.fetch().then((data) => {
        ...
        this.universityOptions = [
          { label: 'All Universities', value: '' },
          ...universities.map(uni => ({ label: uni.university_name, value: uni.university_name }))
        ]
      }).catch((error) => {
    ...
    getActionOptions(suggestion) {
      return [
        {
          label: 'Edit',
          icon: 'edit',
          onClick: () => this.editSuggestion(suggestion)
        },
        {
          label: 'View Master Course',
          icon: 'external-link',
          onClick: () => this.viewMasterCourse(suggestion)
        },
        {
          label: 'Delete',
          icon: 'trash-2',
          onClick: () => this.deleteSuggestion(suggestion)
        }
      ]
    },

    createNew() {
      window.location.href = `/app/course-suggestion/new`
    },

    viewSuggestion(suggestion) {
      window.location.href = `/app/course-suggestion/${suggestion.name}`
    },
    ...
    approveSuggestion(suggestion) {
      if (confirm(`Approve course suggestion: ${suggestion.suggested_course_name}?`)) {
        ...
        resource.fetch().then(() => {
          console.log('Suggestion approved:', suggestion.name)
          this.loadSuggestions() // Reload data
        }).catch((error) => {
    ...
    rejectSuggestion(suggestion) {
      if (confirm(`Reject course suggestion: ${suggestion.suggested_course_name}?`)) {
    ...
    viewMasterCourse(suggestion) {
      if (suggestion.master_course) {
        window.location.href = `/app/course/${suggestion.master_course}`
      }
    },

    deleteSuggestion(suggestion) {
      if (confirm(`Are you sure you want to delete the suggestion for ${suggestion.suggested_course_name}?`)) {
        console.log('Delete suggestion:', suggestion.name)
      }
    }
  }
}
