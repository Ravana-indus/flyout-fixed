<template>
  <div class="min-h-screen bg-gray-50 flex">
      ...
      <header class="bg-white shadow-sm border-b border-gray-200">
        <div class="flex items-center justify-between h-16 px-6">
          ...
          <div class="flex items-center space-x-4">
            <!-- Refresh Button -->
            <Button @click="loadApplications" variant="outline" size="sm" :loading="loading">
      ...
      <main class="p-6">
        ...
        <div v-if="true" class="bg-green-50 border border-green-200 rounded-md p-4 mb-6">
          <div class="flex justify-between items-start">
            <div class="flex-1">
              <h4 class="text-sm font-medium text-green-800">Debug Info - Applications:</h4>
            ...
            <div class="space-x-2 flex-shrink-0">
              <div class="flex flex-col space-y-2">
                <div class="space-x-2">
                  ...
                  <Button @click="loadRealApplicationsNow" size="sm" class="bg-blue-600 text-white">Force Load Real Data</Button>
                </div>
                <div class="space-x-2">
                  <Button @click="loadApplications" size="sm" variant="outline">Retry API</Button>
        ...
        <div class="bg-white rounded-lg shadow">
          <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex items-center justify-between">
              ...
              <div class="flex items-center space-x-2">
                <select v-model="statusFilter" @change="filterApplications" class="text-sm border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500">
                ...
                <select v-model="sortBy" @change="sortApplications" class="text-sm border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500">
                  <option value="date">Sort by Date</option>
                  <option value="name">Sort by Name</option>
                  <option value="status">Sort by Status</option>
                  <option value="service_type">Sort by Service</option>
                </select>
              </div>
            </div>
          </div>
          ...
          <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <tr v-for="application in filteredApplications" :key="application.name">
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="flex items-center">
                      ...
                      <div class="ml-4">
                        <div class="text-sm font-medium text-gray-900">{{ application.client_name || 'Unknown Client' }}</div>
                        <div class="text-sm text-gray-500">{{ application.client_email || 'No email' }}</div>
                      </div>
                    </div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="text-sm text-gray-900">{{ application.service_type || 'N/A' }}</div>
                    <div class="text-sm text-gray-500">{{ application.category || 'General' }}</div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    <div class="flex items-center">
                      <div class="w-16 bg-gray-200 rounded-full h-2 mr-2">
                        <div
                          ...
                          :style="{ width: (application.overall_progress || 0) + '%' }"
                        ></div>
                      </div>
                      <span class="text-sm text-gray-600">{{ application.overall_progress || 0 }}%</span>
                    </div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    <div class="text-sm text-gray-900">${{ (application.total_amount || 0).toLocaleString() }}</div>
                    <div class="text-xs text-gray-500">Paid: ${{ (application.paid_amount || 0).toLocaleString() }}</div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    {{ formatDate(application.application_date || application.creation) }}
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <Button @click="viewApplication(application)" variant="outline" size="sm" class="mr-2">View</Button>
                    <Button @click="updateApplication(application)" size="sm">Update</Button>
                  </td>
                </tr>
              </tbody>
            </table>
          ...
          <div v-if="filteredApplications.length === 0" class="text-center py-12">
            ...
            <div v-if="applications.length === 0" class="mt-4 space-x-2">
    ...
    <!-- Filter Dialog -->
    <Dialog title="Filter Applications" v-model="showFilterDialog">
      <div class="p-6">
...
export default {
  ...
  data() {
    return {
      ...
      navigation: [
        ...
        { name: 'Application', href: '/application', icon: ApplicationIcon, current: true },
    ...
    applications: {
      ...
      params: {
        ...
        fields: [
          ...
          'application_date', 'priority', 'application_status', 'provider', 'service_type',
          'category', 'budget_min', 'budget_max', 'timeline_weeks', 'deadline',
          'expected_completion_date', 'actual_completion_date', 'application_description',
          'overall_progress', 'current_stage', 'total_amount', 'paid_amount',
          'outstanding_amount', 'payment_status', 'source', 'assigned_to',
          'is_urgent', 'confidential', 'creation', 'modified'
        ],
      ...
      auto: false,
      onSuccess(data) {
        console.log('✅ Applications loaded via resource:', data)
        this.applications = data || []
        this.filteredApplications = [...this.applications]
        this.calculateStats()
      },
      onError(error) {
        console.error('❌ Failed to load applications via resource:', error)
        this.applications = []
  ...
  methods: {
    ...
    async loadApplications() {
      this.loading = true
      try {
        console.log('Loading applications using Frappe resource...')
        console.log('Resource object:', this.$resources.applications)

        if (this.$resources.applications && this.$resources.applications.fetch) {
          await this.$resources.applications.fetch()
        } else if (this.$resources.applications && this.$resources.applications.reload) {
          await this.$resources.applications.reload()
        } else {
          console.log('Resource methods available:', Object.keys(this.$resources.applications || {}))
          throw new Error('No fetch or reload method available on resource')
        }
      } catch (error) {
        console.error('Failed to load applications:', error)
        console.log('Falling back to test data...')
        this.applications = this.getTestData()
        this.filteredApplications = [...this.applications]
        this.calculateStats()
      } finally {
        this.loading = false
    ...
    calculateStats() {
      ...
      this.stats.active = this.applications.filter(i => i.application_status === 'In Progress').length
      this.stats.converted = this.applications.filter(i => i.application_status === 'Completed').length

      // Calculate revenue (simple estimation based on budget)
      this.stats.totalRevenue = this.applications.reduce((total, application) => {
        const totalAmount = application.total_amount || 0
        return total + totalAmount
      }, 0)

      // Update navigation count
      const applicationsNav = this.navigation.find(item => item.name === 'Applications')
      if (applicationsNav) {
        applicationsNav.count = this.stats.total
      }
    },
    ...
    filterApplications() {
      if (this.statusFilter === 'all') {
        ...
        this.filteredApplications = this.applications.filter(
          application => application.application_status === this.statusFilter
        )
      }
    ...
    async testApiConnection() {
      ...
      try {
        await this.loadRealApplicationsNow()
      } catch (error) {
    ...
    async loadRealApplicationsNow() {
      console.log('Force loading applications using direct API call...')
      this.loading = true
      ...
      try {
        ...
        if (data && data.message && Array.isArray(data.message)) {
          this.applications = data.message
          this.filteredApplications = [...this.applications]
          this.calculateStats()
          alert(`✅ SUCCESS: Loaded ${this.applications.length} real applications!`)
        } else {
    ...
    loadTestData() {
      console.log('Loading test data for applications...')
      this.applications = this.getTestData()
      this.filteredApplications = [...this.applications]
      this.calculateStats()
    },
  ...
  mounted() {
    console.log('Applications component mounted, loading data...')
...
<style scoped>
/* Custom styles for applications page */
.sidebar-transition {
