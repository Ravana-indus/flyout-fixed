<template>
  <div class="min-h-screen bg-gray-50 flex">
    ...
    <div class="flex-1 lg:pl-0">
      ...
      <header class="bg-white shadow-sm border-b border-gray-200">
        <div class="flex items-center justify-between h-16 px-6">
          <div class="flex items-center">
            <h2 class="text-2xl font-bold text-gray-900">Accepted Inquiries</h2>
          </div>
          <div class="flex items-center space-x-4">
            <!-- Refresh Button -->
            <Button @click="loadAcceptedInquiries" variant="outline" size="sm" :loading="loading">
      ...
      <main class="p-6">
        ...
        <div v-if="true" class="bg-green-50 border border-green-200 rounded-md p-4 mb-6">
          <div class="flex justify-between items-start">
            <div class="flex-1">
              <h4 class="text-sm font-medium text-green-800">Debug Info - Accepted Inquiries:</h4>
              <div class="text-xs text-green-700 mt-2 grid grid-cols-2 gap-4">
                <div>
                  <div><strong>State:</strong></div>
                  <div>Loading: {{ loading }}</div>
                  <div>Accepted Inquiries: {{ acceptedInquiries.length }}</div>
                  <div>Filtered: {{ filteredAcceptedInquiries.length }}</div>
            ...
            <div class="space-x-2 flex-shrink-0">
              <div class="flex flex-col space-y-2">
                <div class="space-x-2">
                  <Button @click="testApiConnection" size="sm" class="bg-green-600 text-white">Test API & Load Real Data</Button>
                  <Button @click="loadRealAcceptedInquiriesNow" size="sm" class="bg-blue-600 text-white">Force Load Real Data</Button>
                </div>
                <div class="space-x-2">
                  <Button @click="loadAcceptedInquiries" size="sm" variant="outline">Retry API</Button>
        ...
        <!-- Accepted Inquiries List -->
        <div class="bg-white rounded-lg shadow">
          <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex items-center justify-between">
              <h3 class="text-lg font-medium text-gray-900">All Accepted Inquiries</h3>
              <div class="flex items-center space-x-2">
                <select v-model="statusFilter" @change="filterAcceptedInquiries" class="text-sm border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500">
                ...
                <select v-model="sortBy" @change="sortAcceptedInquiries" class="text-sm border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500">
                  <option value="date">Sort by Date</option>
                  <option value="name">Sort by Name</option>
                  <option value="status">Sort by Status</option>
                  <option value="conversion_date">Sort by Conversion</option>
                </select>
              </div>
            </div>
          </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gray-50">
                <tr>
                  ...
                  <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Conversion Date</th>
                  <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th>
                  <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th>
                </tr>
              </thead>
              <tbody class="bg-white divide-y divide-gray-200">
                <tr v-for="inquiry in filteredAcceptedInquiries" :key="inquiry.name">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="text-sm text-gray-900">{{ inquiry.provider || 'Not assigned' }}</div>
                    <div class="text-sm text-gray-500">{{ inquiry.priority || 'Medium' }}</div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {{ formatDate(inquiry.conversion_date || inquiry.creation) }}
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <Button @click="viewAcceptedInquiry(inquiry)" variant="outline" size="sm" class="mr-2">View</Button>
                    <Button @click="manageInquiry(inquiry)" size="sm">Manage</Button>
                  </td>
                </tr>
              </tbody>
            </table>
          ...
          <!-- Empty State -->
          <div v-if="filteredAcceptedInquiries.length === 0" class="text-center py-12">
            <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
            </svg>
            <h3 class="mt-2 text-sm font-medium text-gray-900">No accepted inquiries loaded</h3>
            <p class="mt-1 text-sm text-gray-500">
              <span v-if="acceptedInquiries.length === 0">
            ...
            <div v-if="acceptedInquiries.length === 0" class="mt-4 space-x-2">
    ...
    <!-- View Accepted Inquiry Dialog -->
    <Dialog title="Accepted Inquiry Details" v-model="showViewDialog" size="lg">
      <div class="p-6" v-if="selectedInquiry">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <h4 class="text-sm font-medium text-gray-500">Acceptance Details</h4>
            <div class="mt-2 space-y-2">
              ...
              <div><span class="font-medium">Provider:</span> {{ selectedInquiry.provider || 'Not assigned' }}</div>
              <div><span class="font-medium">Priority:</span> {{ selectedInquiry.priority || 'Medium' }}</div>
              <div><span class="font-medium">Conversion Date:</span> {{ formatDate(selectedInquiry.conversion_date) }}</div>
        ...
        <div class="mt-6 flex justify-end space-x-3">
          ...
          <Button @click="manageInquiry(selectedInquiry); showViewDialog = false">Manage</Button>
        </div>
      </div>
    </Dialog>

    <!-- Filter Dialog -->
    <Dialog title="Filter Accepted Inquiries" v-model="showFilterDialog">
      <div class="p-6">
...
// Icon components (reused from Inquiries.vue)
const DashboardIcon = {
...
export default {
  ...
  data() {
    return {
      ...
      navigation: [
        ...
        { name: 'Accepted Inquiries', href: '/accepted-inquiries', icon: AcceptedIcon, current: true, count: 8 },
      ...
      // Accepted Inquiries data
      acceptedInquiries: [],
      filteredAcceptedInquiries: [],
  ...
  resources: {
    acceptedInquiries: {
      ...
      params: {
        doctype: 'Accepted Inquiries',
        fields: [
          'name', 'client_name', 'client_email', 'client_phone', 'client_id',
          'inquiry_date', 'priority', 'status', 'provider', 'description',
          'special_requirements', 'budget_min', 'budget_max', 'currency',
          'budget_flexibility', 'preferred_countries', 'alternative_countries',
          'required_languages', 'alternative_languages', 'conversion_date',
          'conversion_notes', 'selected_provider', 'creation', 'modified'
        ],
      ...
      auto: false,
      onSuccess(data) {
        console.log('✅ Accepted inquiries loaded via resource:', data)
        this.acceptedInquiries = data || []
        this.filteredAcceptedInquiries = [...this.acceptedInquiries]
        this.calculateStats()
      },
      onError(error) {
        console.error('❌ Failed to load accepted inquiries via resource:', error)
        this.acceptedInquiries = []
        this.filteredAcceptedInquiries = []
      }
    },
    user: {
  ...
  methods: {
    ...
    async loadAcceptedInquiries() {
      this.loading = true
      try {
        console.log('Loading accepted inquiries using Frappe resource...')
        console.log('Resource object:', this.$resources.acceptedInquiries)

        if (this.$resources.acceptedInquiries && this.$resources.acceptedInquiries.fetch) {
          await this.$resources.acceptedInquiries.fetch()
        } else if (this.$resources.acceptedInquiries && this.$resources.acceptedInquiries.reload) {
          await this.$resources.acceptedInquiries.reload()
        } else {
          console.log('Resource methods available:', Object.keys(this.$resources.acceptedInquiries || {}))
          throw new Error('No fetch or reload method available on resource')
        }
      } catch (error) {
        console.error('Failed to load accepted inquiries:', error)
        console.log('Falling back to test data...')
        this.acceptedInquiries = this.getTestData()
        this.filteredAcceptedInquiries = [...this.acceptedInquiries]
        this.calculateStats()
      } finally {
        this.loading = false
    ...
    calculateStats() {
      this.stats.total = this.acceptedInquiries.length
      this.stats.active = this.acceptedInquiries.filter(i => i.status === 'Matched').length
      this.stats.converted = this.acceptedInquiries.filter(i => i.status === 'Converted').length

      // Calculate revenue (simple estimation based on budget)
      this.stats.totalRevenue = this.acceptedInquiries.reduce((total, inquiry) => {
        const avgBudget = ((inquiry.budget_min || 0) + (inquiry.budget_max || 0)) / 2
        return total + avgBudget
      }, 0)

      // Update navigation count
      const acceptedNav = this.navigation.find(item => item.name === 'Accepted Inquiries')
      if (acceptedNav) {
        acceptedNav.count = this.stats.total
      }
    },

    filterAcceptedInquiries() {
      if (this.statusFilter === 'all') {
        this.filteredAcceptedInquiries = [...this.acceptedInquiries]
      } else {
        this.filteredAcceptedInquiries = this.acceptedInquiries.filter(
          inquiry => inquiry.status === this.statusFilter
        )
      }
      this.sortAcceptedInquiries()
    },

    sortAcceptedInquiries() {
      this.filteredAcceptedInquiries.sort((a, b) => {
        switch (this.sortBy) {
          case 'name':
          ...
          case 'conversion_date':
            return new Date(b.conversion_date || b.creation) - new Date(a.conversion_date || a.creation)
          case 'date':
          default:
    ...
    viewAcceptedInquiry(inquiry) {
      this.selectedInquiry = inquiry
      this.showViewDialog = true
    },

    manageInquiry(inquiry) {
      console.log('Managing inquiry:', inquiry.name)
      // Implement inquiry management logic
      alert(`Managing inquiry: ${inquiry.name}`)
    },

    async testApiConnection() {
      console.log('🧪 Testing API connection for Accepted Inquiries...')
      this.loading = true

      try {
        await this.loadRealAcceptedInquiriesNow()
      } catch (error) {
    ...
    async loadRealAcceptedInquiriesNow() {
      console.log('Force loading accepted inquiries using direct API call...')
      this.loading = true
      ...
      try {
        ...
        if (data && data.message && Array.isArray(data.message)) {
          this.acceptedInquiries = data.message
          this.filteredAcceptedInquiries = [...this.acceptedInquiries]
          this.calculateStats()
          alert(`✅ SUCCESS: Loaded ${this.acceptedInquiries.length} real accepted inquiries!`)
        } else {
          throw new Error('Invalid response format')
        }
      } catch (error) {
        console.error('❌ Direct API call failed:', error)
        alert(`❌ Failed to load real data: ${error.message}`)
      } finally {
        this.loading = false
      }
    },
    ...
    loadTestData() {
      console.log('Loading test data for accepted inquiries...')
      this.acceptedInquiries = this.getTestData()
      this.filteredAcceptedInquiries = [...this.acceptedInquiries]
      this.calculateStats()
    },
      ...
      return [
        {
          ...
          status: 'Converted',
          provider: 'PROV-001',
          description: 'Immigration services to Canada - successfully completed',
          budget_min: 5000,
          budget_max: 10000,
          currency: 'USD',
          ...
          conversion_notes: 'Successfully converted to application APP-001',
          creation: '2025-01-10 10:30:00'
        },
        {
          name: 'ACC-002',
          ...
          conversion_date: '2025-01-12',
          conversion_notes: 'Matched with provider, proceeding with application',
          creation: '2025-01-08 14:20:00'
        },
        {
          name: 'ACC-003',
          ...
          conversion_notes: 'Successfully completed and converted to application APP-003',
          creation: '2025-01-05 09:15:00'
        }
      ]
    },

    applyFilters() {
      let filtered = [...this.acceptedInquiries]

      if (this.filters.dateFrom) {
        filtered = filtered.filter(inquiry =>
          new Date(inquiry.conversion_date || inquiry.creation) >= new Date(this.filters.dateFrom)
        )
      }

      if (this.filters.dateTo) {
        filtered = filtered.filter(inquiry =>
          new Date(inquiry.conversion_date || inquiry.creation) <= new Date(this.filters.dateTo)
        )
      }
      ...
      if (this.filters.provider) {
        filtered = filtered.filter(inquiry =>
          (inquiry.provider || '').toLowerCase().includes(this.filters.provider.toLowerCase())
        )
      }

      this.filteredAcceptedInquiries = filtered
  ...
  mounted() {
    console.log('AcceptedInquiries component mounted, loading data...')
    this.loadAcceptedInquiries()
  },
  components: {
    Dialog,
  }
...
<style scoped>
/* Custom styles for accepted inquiries page */
.sidebar-transition {
